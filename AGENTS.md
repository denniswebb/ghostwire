...
Unit tests follow table-driven patterns with `t.Parallel()` for concurrency. The discovery package exercises service pairing logic against a fake Kubernetes clientset, the iptables package relies on a `recordingExecutor` mock to validate command sequencing without touching the host firewall, the k8s package uses mock `LabelReader` and `TransitionHandler` implementations to stress the poller, the metrics package verifies Prometheus collectors and HTTP handlers via `httptest` and `prometheus/testutil`, and the watcher command includes focused tests for the `jumpManager` and `metricsLabelReader` wrappers. Integration testing uses KIND clusters (see `/test/kind/`) with sample Service manifests and validation scripts. KIND coverage now spans both the init container (DNAT map generation and rule verification) and the watcher sidecar (label polling, jump rule activation/removal, `/metrics`, `/healthz`). Before opening a PR, run unit tests (`mise run test`), optionally run `act pull_request` for CI rehearsal, and consider the KIND flow when changes affect discovery, iptables, polling, metrics, or watcher glue code. `/test/kind/README.md` documents the full integration workflow, including `deploy-watcher-test.sh` and `test-watcher-transitions.sh` for watcher validation.
